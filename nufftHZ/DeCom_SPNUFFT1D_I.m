function [fhat,ffun] = DeCom_SPNUFFT1D_I(f,x,k,M,gap,tol)
% This code implements the 1D type I forward sparse and partial NUFFT.
...
% M - sparsity in the frequency domain of interest
% gap - frequency gap
...
N = numel(x);
L = floor(N/gap); % points per period
if mod(N,2) < 0.5
    if norm(k-(-N/2:(N/2-1))') < eps
        sft = 1;
        kk = 0:gap:(N/2-1); tmp = -gap:-gap:-N/2; kk = [tmp,kk];
    ...
    if norm(k-(-(N-1)/2:(N-1)/2)') < eps
        sft = 1;
        kk = 0:gap:((N-1)/2); tmp = -gap:-gap:-(N-1)/2; kk = [tmp,kk];
    else if norm(k-(0:N-1)') < eps
            sft = 0;
            kk = 0:gap:(N-1);
        else
...
st = floor(st/Nk)*Nk;
xx = x(st:st+Nk-1);
[L,R,idx] = DeCom_NUFFT1D_I_Fac(xx,kk,tol);
[~,r] = size(L);
Id = sparse(idx,1:Nk,ones(1,Nk),Nk,Nk);
% if sft
%     ffun = @(f) sum(L.*fftshift(spfft(idx,R,f,Nk,N,gap),1),2);
% else
%     ffun = @(f) sum(L.*spfft(Id*(R.*repmat(f,[1,r])),Nk,N,gap),2);
% end
if sft
    ffun = @(f) sum(L.*fftshift(fft(Id*(R.*repmat(f(st:st+Nk-1),[1,r])), [], 1),1),2);
else
    ffun = @(f) sum(L.*fft(Id*(R.*repmat(f(st:st+Nk-1),[1,r])), [], 1),2);
end
fhat = ffun(f);
end

function fhat = spfft(idx,R,f,L,N,gap)
    cnt = randi([1,L],1,1);
    st = ((cnt-1)*L+1);
    pos = st:st+L-1;
    if find(pos>N)
